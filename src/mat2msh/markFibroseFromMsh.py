...
def mark_fibrosis(path_msh, stl_dir, output_path="saida_com_fibrose.msh", plot_centroids=False):
    ...
    n_marked = np.sum(tags_new != tags)
    if n_marked == 0:
        print("No fibrosis was marked. Please check the STL files and ensure they are aligned with the mesh.")
        raise RuntimeError("No fibrosis regions were marked in the mesh.")
